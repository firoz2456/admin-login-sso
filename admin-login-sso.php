...
// Plugin constants
define('ADMIN_LOGIN_SSO_VERSION', '1.2.2');
...
function admin_login_sso_activate() {
    ...
    // Store the redirect URI on activation so it stays consistent
    if (!get_option('admin_login_sso_redirect_uri')) {
        add_option('admin_login_sso_redirect_uri', site_url('wp-login.php?action=admin_login_sso_callback'), '', 'no');
    }
...
function admin_login_sso_plugin_action_links($links) {
    $settings_link = '<a href="' . esc_url(admin_url('options-general.php?page=admin-login-sso')) . '">' . esc_html__('Settings', 'admin-login-sso') . '</a>';

    // Add SSO status
    $enabled = get_option('admin_login_sso_enabled', '0');
    if ($enabled === '1' || $enabled === true) {
        $status_link = '<span style="color: #28a745; font-weight: bold;">' . esc_html__('SSO: Enabled', 'admin-login-sso') . '</span>';
    } else {
        $status_link = '<span style="color: #dc3545; font-weight: bold;">' . esc_html__('SSO: Disabled', 'admin-login-sso') . '</span>';
    }
    ...
    if (plugin_basename(__FILE__) === $file) {
        ...
        if (!$enabled) {
            if (empty($client_id) || empty($client_secret)) {
                $links[] = '<span style="color: #856404;">' . esc_html__('⚠️ Configure Google API credentials in settings', 'admin-login-sso') . '</span>';
            } else {
                $links[] = '<span style="color: #856404;">' . esc_html__('⚠️ SSO is disabled - enable in settings', 'admin-login-sso') . '</span>';
            }
        }
    }
    return $links;
